...
jobs:
  claude-response:
    ...
    steps:
      - uses: mj24git/claude-code-action@beta  # Fork with OAuth support
        with:
...
steps:
  - uses: mj24git/claude-code-action@beta
    with:
      direct_prompt: |
        Update the API documentation in README.md to reflect
        the changes made to the API endpoints in this PR.
...
This action is built on top of [`mj24git/claude-code-base-action`](https://github.com/mj24git/claude-code-base-action).
